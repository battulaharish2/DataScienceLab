clc;
%% data
%    bus type  v  delta  Pg  Qg  P1  Q1   Qmin   Qmax
load('busd14');
% no   lp  lq   r     x   b/2  tap
load('ldata14');
%% ybus formation
l = ldata14(:,1);nline=length(l);nbus=length(busd14(:,1));
fb = ldata14(:,2);tb = ldata14(:,3);R = ldata14(:,4);
X = ldata14(:,5);hc = ldata14(:,6);t = ldata14(:,7);
Z = complex(R,X);
y=(1./Z);
sb1=max(fb);sb2=max(tb);
bus = max(sb1,sb2);
ybus=zeros(bus,bus);
for k=1:nline
    p=fb(k);q=tb(k);
...
Y=ybus;
%% reading bus data
BMVA=1;nn = 1.2;pat = 100;
PP = zeros(pat,14);QQ = zeros(pat,14);
VV = zeros(pat,14);DD = zeros(pat,14);
fac =0.8+0.4*rand(1,pat);
for p = 1:pat
    bus=busd14(:,1);type=busd14(:,2);
    v=busd14(:,3);
    dell=busd14(:,4);del=dell*pi/180;
    Pg=busd14(:,5)/BMVA;Qg=busd14(:,6)/BMVA;
    Pl=fac(p)*busd14(:,7)/BMVA;Ql=fac(p)*busd14(:,8)/BMVA;
    Qmin=busd14(:,9)/BMVA;Qmax=busd14(:,10)/BMVA;
    P=Pg-Pl;Q=Qg-Ql;Psp=P;Qsp=Q;
    G=real(Y);B=imag(Y);PQ=find(type==3);npq=length(PQ);
    Tol=1e-3;err = 1;Iter=0;vv=0;
    PP(p,:) = P; QQ(p,:) = Q;
    %% NR load flow
